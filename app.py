...
# Form Inputs grouped logically into Expanders
with st.expander("👤 1. Demographics & Personal Attributes", expanded=True):
...
with st.expander("💼 2. Employment, Department & Role Details", expanded=False):
    col1, col2, col3 = st.columns(3)
    with col1:
        Sector = st.selectbox(
            "Sector",
...
with st.expander("💰 3. Compensation, Benefits & Overtime", expanded=False):
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    with col2:
        MedicalInsurance = st.selectbox("Medical insurance", ["No", "Yes"])
        Bonus = st.selectbox("Bonus", ["No", "Yes"])
    with col3:
...
with st.expander("📊 4. Workplace Culture & Satisfaction Feedback", expanded=False):
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    with col2:
        Job_Support = st.selectbox("Job support", ["Low", "Medium", "High"])
        Recognition = st.selectbox("Recognition", ["No", "Yes"])
        Emotional_Commitment = st.selectbox("Emotional commitment", ["Low", "Medium", "High"])
        Job_Engagement = st.selectbox("Job engagement", ["Difficult", "Medium", "Easy"])
    with col3:
        Work_Live_Balance = st.selectbox("Work-life balance", ["Difficult", "Medium", "Easy"])
        Physical_Stress = st.selectbox("Physical stress", ["No", "Sometimes", "Yes"])
        Psychological_Exhaustion = st.selectbox("Psychological exhaustion", ["No", "Sometimes", "Yes"])
        Job_Stability = st.selectbox("Job stability", ["No", "Yes"])
        Environment_Satisfaction = st.selectbox("Environment satisfaction", ["Low", "Medium", "High"])
        Job_Satisfaction = st.selectbox(
            "Job satisfaction",
            ["Not satisfied", "Satisfied", "Very satisfied"]
        )
        Job_Opportunities = st.selectbox("Job opportunities", ["No", "Yes"])
...
# Robust attribute extraction for Pipeline models
...
if st.button("🔮 Predict Attrition", type="primary", use_container_width=True):
    ...
    if missing_columns:
        st.error("The following required model features are missing from the input:")
    ...
    if prediction == 1:
        st.error("⚠️ Predicted outcome: High Attrition Risk (Yes)")
        risk = "HIGH"
    else:
        st.success("✅ Predicted outcome: Low Attrition Risk (No)")
        risk = "LOW"
